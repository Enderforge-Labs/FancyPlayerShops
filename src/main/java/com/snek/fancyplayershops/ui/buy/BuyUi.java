...
public class BuyUi extends ShopCanvas implements InputIndicatorCanvas {
    ...
    public boolean attemptChangeAmount(final @NotNull Player user, final float _amount) {
        ...
        if(_amount > Configs.shop.stockLimit.getMax()) {
            user.displayClientMessage(new Txt("The amount cannot be greater than " + Utils.formatAmount(Configs.shop.stockLimit.getMax(), false, true)).red().bold().get(), true);
            return false;
        }
