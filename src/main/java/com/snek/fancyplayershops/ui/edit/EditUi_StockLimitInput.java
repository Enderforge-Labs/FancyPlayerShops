...
public class EditUi_StockLimitInput extends ShopTextInput {
    ...
    public EditUi_StockLimitInput(final @NotNull Shop _shop) {
        super(_shop, null, "Change stock limit", new Txt("Send the new stock limit in chat!").color(FancyPlayerShops.SHOP_ITEM_NAME_COLOR).bold().get(), new EditUi_Input_S(_shop));
        updateDisplay(null);
    }
